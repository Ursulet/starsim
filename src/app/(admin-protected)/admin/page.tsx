  ...
  return (
    <div>
      ...
      <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        <AdminStatCard label="Programe" value={data.programs} /><AdminStatCard label="Evenimente" value={data.events} /><AdminStatCard label="Articole" value={data.articles} /><AdminStatCard label="Mesaje contact" value={data.messages} /><AdminStatCard label="Abonati newsletter" value={data.subscribers} /><AdminStatCard label="Imagini media" value={data.media} />
      </div>
      <h2 className="mt-10 text-xl font-bold text-starsim-navy">Prioritati etapa curenta</h2>
      <div className="mt-4 grid gap-4 md:grid-cols-3">{["Configureaza continutul", "Pregateste SEO", "Publica primele evenimente"].map((item) => <div key={item} className="rounded-2xl border border-slate-200 bg-white p-5 text-slate-600 shadow-sm">{item}</div>)}</div>
    </div>
  );
}
